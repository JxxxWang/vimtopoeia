...
def generate_sample(
    ...
    return VSTDataSample(
        ...
        # target_mel_spec=target_spectrogram,
        # reference_mel_spec=ref_spectrogram,
...
def create_datasets_and_get_start_idx(
    ...
    target_param_ds, tp_idx = create_dataset_and_get_first_unwritten_idx(
        hdf5_file, "target_param_array", param_shape, np.float32, hdf5plugin.Blosc2()
    )
    ref_param_ds, rp_idx = create_dataset_and_get_first_unwritten_idx(
        hdf5_file, "reference_param_array", param_shape, np.float32, hdf5plugin.Blosc2()
    )
